...
MODEL:
```{r}
nba_glm <- glm(win ~ FG3A  + log(pts_allowed) + FTA  + OREB + DREB + TO + SEASON, data = final_data, family = "binomial")
summary(nba_glm)
```
MULTI COLLINEARITY:
```{r}
final_data |> 
  ggpairs(columns = c("win", "FG3A", "pts_allowed", "FTA", "OREB", "DREB", "TO", "SEASON"))
vif(nba_glm)
```
No evidence of significant multicollinearity.

OUTLIERS:
```{r}
plot(rstandard(nba_glm), main="Standardized Residuals", ylab="Standardized Residuals", xlab="Index")
plot(hatvalues(nba_glm), main="Hat Values", ylab="Standardized Residuals", xlab="Index")
plot(cooks.distance(nba_glm), main="Cooks D", ylab="Standardized Residuals", xlab="Index") 
cooks_d <- cooks.distance(nba_glm)
